...
import (
	...
	"github.com/cloudwego-contrib/obs-opentelemetry/instrumentation/otelhertz"
	"github.com/cloudwego-contrib/obs-opentelemetry/instrumentation/otelkitex"
...
// Inject injects span context into the otelhertz metadata info
func InjectHTTP(ctx context.Context, c *otelhertz.Config, headers *protocol.RequestHeader) {
	c.GetTextMapPropagator().Inject(ctx, getMetadataProviderHTTP(headers))
}

// Extract returns the baggage and span context
func ExtractHTTP(ctx context.Context, c *otelhertz.Config, headers *protocol.RequestHeader) (baggage.Baggage, trace.SpanContext) {
...
// Inject injects span context into the otelkitex metadata info
func InjectRPC(ctx context.Context, c *otelkitex.Config, metadata map[string]string) {
	c.GetTextMapPropagator().Inject(ctx, getMetadataProviderRPC(metadata))
}

// Extract returns the baggage and span context
func ExtractRPC(ctx context.Context, c *otelkitex.Config, metadata map[string]string) (baggage.Baggage, trace.SpanContext) {
