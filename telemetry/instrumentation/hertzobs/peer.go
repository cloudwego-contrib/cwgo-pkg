...
func extractPeerServiceAttributesFromMetadata(headers *protocol.RequestHeader) []attribute.KeyValue {
	...
	if serviceNamespace != "" {
		attrs = append(attrs, semantic.PeerServiceNamespaceKey.String(serviceNamespace))
	}

	if deploymentEnv != "" {
		attrs = append(attrs, semantic.PeerDeploymentEnvironmentKey.String(deploymentEnv))
	}
