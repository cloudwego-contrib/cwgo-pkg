...
func (svr *polarisRegistry) Register(info *registry.Info) error {
	...
	param, instanceKey, err := createRegisterParam(info, svr.so)
	if err != nil {
		return err
	}
...
func createRegisterParam(info *registry.Info, so ServerOptions) (*api.InstanceRegisterRequest, string, error) {
	...
	instanceKey := common.GetInstanceKey(namespace, info.ServiceName, instanceHost, strconv.Itoa(instancePort))
