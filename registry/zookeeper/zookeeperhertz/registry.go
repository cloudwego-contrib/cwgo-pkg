...
func buildPath(info *registry.Info) (string, error) {
	...
	path = path + common.Separator + net.JoinHostPort(host, port)

	return path, nil
}

func (z *zookeeperRegistry) createNode(path string, content []byte, ephemeral bool) error {
	i := strings.LastIndex(path, common.Separator)
	if i > 0 {
	...
	if err != nil {
		return fmt.Errorf("create node [%s] cwerror, cause %w", path, err)
	}
	return nil
}
...
func (z *zookeeperRegistry) deleteNode(path string) error {
	...
	if err != nil && err != zk.ErrNoNode {
		return fmt.Errorf("delete node [%s] cwerror, cause %w", path, err)
	}
	return nil
}
