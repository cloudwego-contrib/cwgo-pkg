...
func (e *etcdRegistry) register(info *registry.Info, leaseID clientv3.LeaseID) error {
	...
	val, err := sonic.Marshal(&internal.InstanceInfo{
	...
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*3)
	defer cancel()
	_, err = e.etcdClient.Put(ctx, internal.ServiceKey(etcdPrefix, info.ServiceName, addr), string(val), clientv3.WithLease(leaseID))
	if err != nil {
		return err
	}
	...
	}(internal.ServiceKey(etcdPrefix, info.ServiceName, addr), string(val))
...
func (e *etcdRegistry) deregister(info *registry.Info) error {
	...
	_, err = e.etcdClient.Delete(ctx, internal.ServiceKey(etcdPrefix, info.ServiceName, addr))
	if err != nil {
		return err
	}
	e.stop <- struct{}{}
...
func (e *etcdRegistry) getAddressOfRegistration(info *registry.Info) (string, error) {
	...
	if host == "" || host == "::" {
		host, err = internal.GetLocalIPv4Host()
		if err != nil {
