...
func TestRegistry(t *testing.T) {
	...
	for _, tes := range tests {
		...
		for _, info := range tes.info {
			...
			assert.False(t, err != nil)
			key := internal.ServiceKey(etcdPrefix, info.ServiceName, info.Addr.String())
			...
			val := kv.Kvs[0].Value
			en := new(internal.InstanceInfo)
			if err := json.Unmarshal(val, en); err != nil {
				t.Errorf("json unmarshal cwerror")
			}
...
func TestResolver(t *testing.T) {
	...
	for _, tes := range tests {
		...
		for _, args := range tes.info.args {
			ctx, cancel := context.WithTimeout(context.Background(), timeout)
			key := internal.ServiceKey(etcdPrefix, info.ServiceName, args.Addr)
			addr := utils.NewNetAddr("tcp", args.Addr)
			content, err := json.Marshal(&internal.InstanceInfo{
