...
func (e *etcdRegistry) register(info *registry.Info, leaseID clientv3.LeaseID) error {
	...
	val, err := json.Marshal(&internal.InstanceInfo{
		Network: network,
		Address: addr,
		Weight:  info.Weight,
		Tags:    info.Tags,
	...
	defer cancel()
	_, err = e.etcdClient.Put(ctx, internal.ServiceKey(e.prefix, info.ServiceName, addr), string(val), clientv3.WithLease(leaseID))
	if err != nil {
		return err
	}
	...
	}(internal.ServiceKey(e.prefix, info.ServiceName, addr), string(val))
...
func (e *etcdRegistry) deregister(info *registry.Info) error {
	...
	_, err = e.etcdClient.Delete(ctx, internal.ServiceKey(e.prefix, info.ServiceName, addr))
	if err != nil {
		return err
	}
	e.stop <- struct{}{}
