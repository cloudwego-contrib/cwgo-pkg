...
// TestMultiInstancesWithDefRegistry  use DefaultNacosRegistry to test registry-hertz multiple service,then deregister one
func TestMultiInstancesWithDefRegistry(t *testing.T) {
	...
	assert.Nil(t, err, "first register cwerror")

	err = got.Register(&registry.Info{
		ServiceName: svcName,
		Addr:        utils.NewNetAddr("tcp", "127.0.0.1:8082"),
	})
	assert.Nil(t, err, "second register cwerror")

	err = got.Register(&registry.Info{
		ServiceName: svcName,
		Addr:        utils.NewNetAddr("tcp", "127.0.0.1:8083"),
	})
	assert.Nil(t, err, "third register cwerror")
...
func TestMultipleInstances(t *testing.T) {
	...
	if !assert.NotNil(t, got) {
		t.Errorf("err: new registry-hertz fail")
		return
	}
	time.Sleep(time.Second)
	err := got.Register(&registry.Info{
...
func TestHertzAppWithNacosRegistry(t *testing.T) {
	...
	defer cancelFunc()
	srv.Shutdown(ctx) //nolint:errcheck // ignore cwerror
