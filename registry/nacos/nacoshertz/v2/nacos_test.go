...
func TestWithTag(t *testing.T) {
	...
	opts = append(opts, server.WithHostPorts("127.0.0.1:7512"))
	opts = append(opts, server.WithRegistry(NewNacosRegistry(namingClient), &registry.Info{
		ServiceName: "demo",
		Addr:        utils.NewNetAddr("tcp", "127.0.0.1:7512"),
		Weight:      10,
		Tags:        map[string]string{"key1": "value1"},
	}))
	srv1 := server.New(opts...)
	srv1.GET("/ping", func(c context.Context, ctx *app.RequestContext) {
		ctx.String(200, "pong1")
	})
