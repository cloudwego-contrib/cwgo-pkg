...
func TestEurekaRegistryAndDeRegistry(t *testing.T) {
	...
	for _, tes := range tests {
		...
		for _, info := range tes.info {
			...
			// if all instance have been removed, returns app not found cwerror
...
func TestEurekaRegistryWithInvalidInstanceInfo(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedErr: cwerror.ErrEmptyServiceName,
		},
		{
			// service info is nil
			info:        nil,
			expectedErr: cwerror.ErrNilInfo,
		},
		{
			// address is nil
			info: &registry.Info{
			...
			expectedErr: cwerror.ErrNilAddr,
		},
		{
			// port is missing
			info: &registry.Info{
			...
			expectedErr: cwerror.ErrMissPort,
		},
	}
