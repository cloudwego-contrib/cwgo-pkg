...
import (
	...
	"github.com/cloudwego-contrib/obs-opentelemetry/cwmetrics/otel/hertz"
	"github.com/cloudwego-contrib/obs-opentelemetry/cwmetrics/otel/kitex"
...
func TestInjectHTTP(t *testing.T) {
	cfg := hertz.NewConfig([]hertz.Option{hertz.WithTextMapPropagator(propagation.NewCompositeTextMapPropagator(
		b3.New(),
		ot.OT{},
		propagation.Baggage{},
...
func TestExtractRPC(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx: ctx,
				c:   kitex.NewConfig([]kitex.Option{}),
				metadata: map[string]string{
					"foo": "bar",
				},
			},
...
func TestInjectRPC(t *testing.T) {
	cfg := kitex.NewConfig([]kitex.Option{kitex.WithTextMapPropagator(propagation.NewCompositeTextMapPropagator(
		b3.New(),
		ot.OT{},
		propagation.Baggage{},
		propagation.TraceContext{},
